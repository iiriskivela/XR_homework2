        ...
        for (int i = 0; i < _rullers.Count; i++)
        {
            if (_isActveEmission)
            {
                if (_changeRuller == i)
                {
                    ...
                    PadLockEmissionColor emissionColorComponent = _rullers[i].GetComponent<PadLockEmissionColor>();
                        if (emissionColorComponent != null) {
                            emissionColorComponent._isSelect = true;
                            // Add debug log to indicate success
                            Debug.Log("PadLockEmissionColor component found and _isSelect set to true for GameObject: " + _rullers[i].name);
                        } else {
                            Debug.LogError("PadLockEmissionColor component is missing on GameObject: " + _rullers[i].name);
                        }
